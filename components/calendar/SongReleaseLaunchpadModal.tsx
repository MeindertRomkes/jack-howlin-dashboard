...
export interface SongReleaseLaunchpadModalProps {
  isOpen: boolean
  onClose: () => void
  onSaved: () => void
}
...
export function SongReleaseLaunchpadModal({
  isOpen,
  onClose,
  onSaved,
}: SongReleaseLaunchpadModalProps) {
            ...
            <div>
              <h2 className="text-base font-extrabold tracking-wider uppercase text-stone-100 flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/60 px-2 py-0.5 rounded uppercase">
                  Rollout &amp; Visual Studio
                </span>
              </h2>
              <p className="text-xs text-stone-400">
                Laat Gemini een complete social release-campagne genereren en maak direct 10s teasers in AI Studio
              </p>
            </div>
          </div>
          <button
                ...
                >
                  {generating ? (
                    ...
                    <>
                      ...
                      <span>Genereer 14-Dagen Rollout ➔</span>
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="bg-stone-950 border border-stone-800 p-4 rounded-xl flex items-center justify-between flex-wrap gap-3">
                <div>
                  <h3 className="text-sm font-bold text-stone-100 flex items-center gap-2">
                    ...
                    <span>14-Dagen Rollout voor &ldquo;{songTitle}&rdquo;</span>
                  </h3>
                ...
                {campaign.map((post, idx) => {
                  return (
                    ...
                    >
                      ...
                      {/* Visual Hook Prompt & Direct 10s Clip Studio Bridge */}
                      <div className="bg-stone-900/70 border border-stone-800 rounded-lg p-3 space-y-2">
                        {post.visualHookPrompt && (
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button
                              ...
                              className="text-[10px] text-stone-400 hover:text-stone-200 font-medium flex items-center gap-1 bg-stone-950 px-2 py-1 rounded border border-stone-800 flex-shrink-0"
                            >
                              {copiedIndex === idx ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                              <span>{copiedIndex === idx ? 'Gekopieerd' : 'Kopieer'}</span>
                            </button>
                          </div>
                        )}

                        <div className="flex items-center justify-between pt-1 border-t border-stone-800/60 flex-wrap gap-2">
                          <span className="text-[10px] text-stone-500 font-mono">
                            Studio Bridge: 10s Outlaw teaser visual
                          </span>
                          <Link
                            href={`/studio?songTitle=${encodeURIComponent(songTitle)}&promptSuggestion=${encodeURIComponent(post.caption)}`}
                            className="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 hover:text-amber-300 border border-amber-500/30 text-[11px] font-bold px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 shadow-sm active:scale-95"
                            title="Genereer direct een 10s teaser video in AI Content Studio"
                          >
                            <Clapperboard className="w-3.5 h-3.5 text-amber-400" />
                            <span>⚡ Genereer 10s Teaser Video</span>
                          </Link>
                        </div>
                      </div>
                    </div>
...
export default SongReleaseLaunchpadModal
